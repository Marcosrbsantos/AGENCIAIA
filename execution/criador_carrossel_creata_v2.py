...
.orange-bar {{ position: absolute; right: 0; top: 0; width: 50px; height: 100%; background-color: #FCA311; }}
.title-container {{ position: absolute; top: 220px; left: 80px; max-width: 580px; z-index: 20; }}
.title {{ font-size: 150px; font-weight: 900; letter-spacing: -6px; background: linear-gradient(90deg, #1A1A1A 0%, #1A1A1A 20%, #E68A00 50%, #FDF5E6 90%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0; line-height: 0.9; }}
.body-text {{ font-size: 32px; font-weight: 400; color: #1A1A1A; line-height: 1.35; margin-top: 30px; }}
.body-text strong {{ color: #1A1A1A; font-weight: 800; }}
.sub-text {{ font-size: 26px; font-weight: 800; color: #D68311; margin-top: 40px; }}
.small-texts {{ font-size: 28px; font-weight: 700; color: #E6A755; margin-top: 40px; font-style: italic; line-height: 1.6; transform: rotate(-3deg); }}
.statue {{ position: absolute; bottom: -30px; right: 80px; width: 500px; z-index: 10; filter: drop-shadow(-10px 10px 15px rgba(0,0,0,0.2)); }}
.arrow-btn {{ display: inline-block; border: 3px solid #1A1A1A; border-radius: 8px; padding: 5px 15px; margin-top: 30px; font-weight: 900; font-size: 26px; color: #1A1A1A; }}
</style>
</head>
<body>
...
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800;900&display=swap');
body {{ margin: 0; padding: 0; width: 1080px; height: 1080px; background-color: #FCA311; font-family: 'Inter', sans-serif; position: relative; overflow: hidden; }}
.bg-text {{ position: absolute; font-size: 250px; font-weight: 900; color: rgba(0,0,0,0.04); top: 0; left: -80px; line-height: 0.8; letter-spacing: -8px; z-index: 1; word-wrap: break-word; }}
.card {{ position: absolute; top: 100px; left: 80px; max-width: 600px; z-index: 20; padding-top: 100px; }}
.card-text {{ font-size: 40px; font-weight: 400; color: #332200; line-height: 1.35; }}
.card-text strong {{ color: #FDF5E6; font-weight: 700; text-shadow: 0 0 10px rgba(255,255,255,0.2); }}
.quotes {{ margin-top: 50px; font-size: 32px; color: #FFE3B3; text-align: left; line-height: 1.6; font-weight: 600; padding-left: 20px; }}
.buttons-row {{ margin-top: 60px; display: flex; gap: 15px; font-size: 26px; font-weight: 600; color: #B36800; }}
.btn-ativo {{ color: #FDF5E6; background: rgba(0,0,0,0.1); padding: 5px 15px; border-radius: 8px; }}
.bottom-text {{ font-size: 26px; font-weight: 700; color: #FFE3B3; transform: rotate(-5deg); margin-top: 60px; margin-left: 20px; }}
.statue {{ position: absolute; right: -50px; top: 250px; width: 550px; z-index: 10; filter: drop-shadow(-20px 20px 30px rgba(0,0,0,0.4)); }}
.arrow-btn {{ display: inline-block; border: 3px solid #332200; border-radius: 8px; padding: 5px 15px; margin-top: 40px; margin-left:150px; font-weight: 900; font-size: 26px; color:#332200; }}
</style>
</head>
<body>
...
body {{ margin: 0; padding: 0; width: 1080px; height: 1080px; background-color: #FCA311; font-family: 'Inter', sans-serif; position: relative; overflow: hidden; }}
.bg-text {{ position: absolute; font-size: 250px; font-weight: 900; color: rgba(0,0,0,0.04); top: 0; left: -80px; line-height: 0.8; letter-spacing: -8px; z-index: 1; word-wrap: break-word; }}
.card {{ position: absolute; top: 150px; left: 80px; max-width: 550px; z-index: 20; }}
.text-main {{ font-size: 45px; font-weight: 400; color: #332200; line-height: 1.25; letter-spacing: -1px; }}
.text-main strong {{ color: #FDF5E6; }}
.huge-text {{ font-size: 64px; font-weight: 900; color: #FDF5E6; letter-spacing: -2px; margin: 15px 0; }}
.small-desc {{ font-size: 22px; color: #332200; line-height: 1.4; margin-top: 20px; font-weight:600; }}
.orange-highlight {{ font-size: 28px; font-weight: 700; color: #FFE3B3; margin-top: 40px; }}
.statue {{ position: absolute; right: -50px; bottom: -50px; width: 550px; z-index: 10; filter: drop-shadow(-20px 20px 30px rgba(0,0,0,0.4)); }}
.arrow-btn {{ display: inline-block; border: 3px solid #332200; border-radius: 8px; padding: 5px 15px; margin-top: 60px; margin-left: 200px; font-weight: 900; font-size: 26px; color:#332200; }}
</style>
</head>
<body>
...
.rotated-text {{ position: absolute; right: 0; top: 250px; font-size: 28px; font-weight: 800; color: #FDF5E6; transform: rotate(15deg); line-height: 1.2; text-shadow: 0 0 20px rgba(255,255,255,0.4); }}
...
async def generate_all_images():
    ...
    os.makedirs("c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v2", exist_ok=True)
    ...
    async with async_playwright() as p:
        ...
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v2/slide_01.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide2.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v2/slide_02.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide3.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v2/slide_03.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide4.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v2/slide_04.png", type="png")
        
        await browser.close()
        
    print("✅ Carrossel Creata v2 gerado com sucesso!")
