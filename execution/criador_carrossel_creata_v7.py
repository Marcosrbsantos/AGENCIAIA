...
.title {{ font-size: 100px; font-weight: 900; letter-spacing: -4px; background: linear-gradient(90deg, #1A1A1A 0%, #1A1A1A 30%, #D68311 60%, rgba(214,131,17,0.3) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0; line-height: 0.9; text-transform: uppercase; white-space: nowrap; }}
...
.quotes-container {{ position: absolute; top: 760px; left: 100px; z-index: 50; /* Aumentado o z-index para ficar acima da estatua */ }}
.small-texts {{ font-size: 28px; font-weight: 700; color: #E6A755; font-style: italic; line-height: 1.6; transform: rotate(-3deg); display: inline-block; vertical-align: top; text-shadow: 0 0 10px rgba(253,245,230,0.8); }}
.arrow-btn {{ display: inline-block; border: 3px solid #1A1A1A; border-radius: 8px; padding: 5px 15px; margin-top: 50px; margin-left: 50px; font-weight: 900; font-size: 26px; color: #1A1A1A; vertical-align: top; background: #FDF5E6; }}
...
/* A caixa agora tem um padding menor pra se juntar mais ao fundo plano (Flat Design do botao) */
...
/* Botao Unico Solicitado em Flat */
.btn-ativo {{ color: #FDF5E6; background: rgba(0,0,0,0.15); padding: 12px 25px; border-radius: 8px; border: 2px solid rgba(0,0,0,0.05); font-weight: 800; box-shadow: none; text-transform: lowercase; }}
...
<div class="card">
    ...
    <div class="buttons-row">
        <span class="btn-ativo">consome o dia</span>
    </div>
</div>
...
def generate_html_slide3():
    # Nova imagem do Celular Flutuante HQ gerada
    img_warrior = "file:///" + os.path.abspath("C:/Users/Admin/.gemini/antigravity/brain/bb885d9d-17c4-480d-8b96-bc832cb3e051/modern_smartphone_dark_1773513014910.png").replace("\\", "/")
    return f"""<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
.statue {{ position: absolute; right: -250px; bottom: 0px; width: 1000px; z-index: 10; filter: drop-shadow(-30px 30px 50px rgba(0,0,0,0.9)); transform: rotate(-5deg); }}
...
.center-content {{ position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 10; width: 850px; }}
.main-quote {{ text-align: center; color: #FCA311; font-size: 60px; margin-top: 40px; line-height: 1.25; font-weight: 700; letter-spacing: -2px; text-transform: uppercase; }}
.small-white {{ text-align: center; color: #FDF5E6; font-size: 30px; margin-top: 80px; font-weight: 500; letter-spacing: -1px; }}
.rotated-text {{ position: absolute; right: 0px; top: -50px; font-size: 32px; font-weight: 800; color: #FDF5E6; transform: rotate(15deg); line-height: 1.2; text-shadow: 0 0 20px rgba(255,255,255,0.4); }}
...
async def generate_all_images():
    ...
    os.makedirs("c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v7", exist_ok=True)
    ...
    async with async_playwright() as p:
        ...
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v7/slide_01.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide2.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v7/slide_02.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide3.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v7/slide_03.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide4.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v7/slide_04.png", type="png")
        
        await browser.close()
        
    print("✅ Carrossel Creata v7 gerado com sucesso!")
