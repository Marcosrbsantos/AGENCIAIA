...
.title-container {{ position: absolute; top: 120px; left: 100px; max-width: 650px; z-index: 20; }}
.title {{ font-size: 160px; font-weight: 900; letter-spacing: -6px; background: linear-gradient(90deg, #1A1A1A 0%, #1A1A1A 30%, #D68311 60%, rgba(214,131,17,0.2) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0; line-height: 0.9; }}
.body-text {{ font-size: 34px; font-weight: 400; color: #1A1A1A; line-height: 1.35; margin-top: 30px; max-width: 580px; }}
.body-text strong {{ color: #1A1A1A; font-weight: 800; }}
.sub-text {{ font-size: 26px; font-weight: 800; color: #D68311; margin-top: 40px; }}
.small-texts {{ font-size: 28px; font-weight: 700; color: #E6A755; margin-top: 40px; font-style: italic; line-height: 1.6; transform: rotate(-3deg); display: inline-block; vertical-align: top; }}
.statue {{ position: absolute; bottom: -50px; right: -180px; width: 900px; z-index: 10; filter: drop-shadow(-15px 15px 25px rgba(0,0,0,0.3)); }}
.arrow-btn {{ display: inline-block; border: 3px solid #1A1A1A; border-radius: 8px; padding: 5px 15px; margin-top: 90px; margin-left: 50px; font-weight: 900; font-size: 26px; color: #1A1A1A; vertical-align: top; }}
</style>
</head>
<body>
<div class="orange-bar"></div>
<div class="title-container">
    <h1 class="title">questione</h1>
    <div class="body-text">
...
.bg-text {{ position: absolute; font-size: 250px; font-weight: 900; color: rgba(255,255,255,0.02); top: 0; left: -80px; line-height: 0.8; letter-spacing: -8px; z-index: 1; word-wrap: break-word; }}
.card {{ position: absolute; top: 150px; left: 80px; bottom: 150px; max-width: 650px; background-color: #FCA311; border-radius: 60px 0 0 60px; z-index: 5; padding: 100px 100px 80px 80px; box-sizing: border-box; box-shadow: -20px 0 50px rgba(0,0,0,0.5); display: flex; flex-direction: column; justify-content: center; }}
...
.buttons-row {{ margin-top: 50px; display: flex; gap: 15px; font-size: 26px; font-weight: 600; color: #B36800; }}
.btn-ativo {{ color: #FDF5E6; background: rgba(0,0,0,0.1); padding: 5px 15px; border-radius: 8px; }}
.bottom-text {{ font-size: 24px; font-weight: 700; color: #FFE3B3; transform: rotate(-5deg); margin-top: 50px; margin-left: 20px; }}
...
.arrow-btn {{ display: inline-block; border: 3px solid #332200; border-radius: 8px; padding: 5px 15px; margin-top: 50px; align-self: center; font-weight: 900; font-size: 26px; color:#332200; }}
</style>
</head>
<body>
...
async def generate_all_images():
    ...
    os.makedirs("c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v3", exist_ok=True)
    ...
    async with async_playwright() as p:
        ...
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v3/slide_01.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide2.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v3/slide_02.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide3.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v3/slide_03.png", type="png")
        
        await page.goto("file:///" + os.path.abspath("c:/Users/Admin/Desktop/Agente/execution/temp_slide4.html").replace("\\", "/"))
        await page.wait_for_timeout(2000)
        await page.screenshot(path="c:/Users/Admin/Desktop/Agente/outputs/carrossel_creata_v3/slide_04.png", type="png")
        
        await browser.close()
        
    print("✅ Carrossel Creata v3 gerado com sucesso!")
